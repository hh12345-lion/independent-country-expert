...
export function NavDropdown({ label, href, items, scrollable }: NavDropdownProps) {
  return (
    <div className="group relative">
      <Link
        href={href}
        className="inline-flex min-h-[44px] items-center gap-1 rounded-[4px] px-2 py-2 text-sm text-[#374151] hover:bg-[#F5F7FA] hover:text-[#2C3E50]"
      >
        {label}
        ...
        >
          {items.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className="block px-4 py-2.5 text-sm text-[#374151] hover:bg-[#F5F7FA] hover:text-[#2C3E50]"
              >
                {item.label}
              </Link>
            </li>
